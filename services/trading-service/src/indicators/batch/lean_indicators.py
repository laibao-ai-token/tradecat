...
def calc_adx(high: np.ndarray, low: np.ndarray, close: np.ndarray, period: int = 14) -> dict:
    ...
    # DI
    # NOTE: avoid np.where(divide) because both branches are evaluated and can emit RuntimeWarning.
    plus_di = np.divide(
        100.0 * smooth_plus,
        smooth_tr,
        out=np.zeros_like(smooth_tr, dtype=float),
        where=smooth_tr > 0,
    )
    minus_di = np.divide(
        100.0 * smooth_minus,
    ...
    dx = np.divide(
        ...
        di_sum,
        out=np.zeros_like(di_sum, dtype=float),
        where=di_sum > 0,
    )
