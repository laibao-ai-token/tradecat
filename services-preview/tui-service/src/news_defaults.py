"""Built-in live news source defaults and source metadata for the TUI news page."""
...
CORE_TUI_NEWS_RSS_FEEDS: tuple[str, ...] = (
    "direct://jin10",
...
DEFAULT_TUI_NEWS_RSS_FEEDS: tuple[str, ...] = _merge_unique(CORE_TUI_NEWS_RSS_FEEDS, WORLDMONITOR_TRADING_RSS_FEEDS)

TUI_NEWS_RSS_PRESETS: dict[str, tuple[str, ...]] = {
    "default": DEFAULT_TUI_NEWS_RSS_FEEDS,
    "core": CORE_TUI_NEWS_RSS_FEEDS,
    "worldmonitor_trading": WORLDMONITOR_TRADING_RSS_FEEDS,
}

_NEWS_FEED_DESCRIPTORS: tuple[NewsFeedDescriptor, ...] = (
    *(NewsFeedDescriptor(feed=feed, code=news_source_code(feed), group=CORE_GROUP, tier=PRIMARY_TIER) for feed in CORE_TUI_NEWS_RSS_FEEDS),
...
def tui_news_rss_feeds_for_preset(preset_value: str | None = None) -> list[str]:
    feeds: list[str] = []
    seen: set[str] = set()
    ...
    for name in preset_names:
        preset = TUI_NEWS_RSS_PRESETS.get(name)
        if not preset:
            continue
        matched = True
        for feed in preset:
    ...
    if matched:
        return feeds
    return list(DEFAULT_TUI_NEWS_RSS_FEEDS)


def default_tui_news_rss_feeds() -> list[str]:
    return tui_news_rss_feeds_for_preset("default")


def default_tui_news_rss_feeds_value(preset_value: str | None = None) -> str:
    return ",".join(tui_news_rss_feeds_for_preset(preset_value))
